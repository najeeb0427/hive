...
// This method takes the input tempratures(TMAX and TMIN) and convert it to tempreture in celsius and fehrenheit
  def getTemperatures(temp:String):String={
...
// Defining all element values in a list
...
// Defining a null sequence for all the Dates. This will be used for those elements which are not present	
	val blankSeq = 0 to 30 map(x => "\\N")
	
// Creating Pair RDD of stations data	
...
//    joined.foreach(println)
    ...
    val flatted_value = grouped.flatMap{
      x =>{
        ...
        1 to 31 map {y => (x._1._1.concat(",").concat(x._1._2.concat("-").concat(x._1._3).concat("-")
          .concat(String.valueOf(y)))
          .concat(",").concat(x._1._4)
          .concat(",").concat(x._1._5)
          .concat(",").concat(getTemperatures(valuesMap.getOrElse("TMAX",blankSeq)(y-1)))
          .concat(",").concat(getTemperatures(valuesMap.getOrElse("TMIN",blankSeq)(y-1)))
          .concat(",").concat(elementValues.map(z => valuesMap.getOrElse(z,blankSeq)(y-1)).mkString(",")))
          }
      }
    }
